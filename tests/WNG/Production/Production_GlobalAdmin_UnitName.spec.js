...
test.describe('Dashboard Validation', () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    await page.goto(process.env.GLOBAL_PROD_URL);
    ...
    await page.getByRole('textbox', { name: 'Login email' }).pressSequentially(process.env.GP_ADMIN_USERNAME, { delay: 100 });
    await page.getByRole('textbox', { name: 'Password' }).pressSequentially(process.env.GP_ADMIN_PASSWORD, { delay: 100 });
  ...
  test('Validate Dashboard DB vs UI', async ({ page }) => {
    ...
    // Select Unit
    const unitName = 'Asyaport'; // dynamic unit name
