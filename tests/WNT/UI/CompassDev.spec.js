...
const COMPASS_DEV_API_URL = process.env.COMPASS_DEV_URL;
    ...
    test('Validate UI data with DB for Complex', async ({ page }) => {
        ...
        if (LOGIN_OTP) {
            ...
            await Promise.race([
                ...
                page.waitForURL(
                    /bamcotest\.com|compassmanager\.com/i,
                    { timeout: 240000 }
                ),
                ...
                page.waitForSelector('#wastenot-html', {
                    timeout: 240000
                })
...
`;
        ...
        const emails = [
            'aditya.parmar@ccube.com'
        ];

        const emailTextbox = page
            .locator('#wastenot-html')
            .contentFrame()
            .getByRole('textbox', { name: 'test@compass-usa.com' });

        //Clear existing emails
        await emailTextbox.click();
        await emailTextbox.press(process.platform === 'darwin' ? 'Meta+A' : 'Control+A');
        await emailTextbox.press('Backspace');
