...
test.describe('Dashboard Validation', () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    await page.getByRole('textbox', { name: 'Login email' }).pressSequentially(process.env.GD_ADMIN_USERNAME, { delay: 100 });
    await page.getByRole('textbox', { name: 'Password' }).pressSequentially(process.env.GD_ADMIN_PASSWORD, { delay: 100 });
  ...
  test('Validate Dashboard DB vs UI', async ({ page }) => {
    ...
    // Select Unit
    const idNumber = 323;
    const unitName = `Unit one (${idNumber})`;
    const unitN = 'Unit One';

    console.log(unitName);  // Unit one (323)
    ...
    const entityUnitQuery = `
      SELECT id 
      FROM entity_units 
      WHERE name = '${unitN}' AND id_number = '${idNumber}'
      LIMIT 1;
    `;
